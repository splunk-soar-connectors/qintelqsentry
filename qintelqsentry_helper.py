...
DEFAULT_HEADERS = {"User-Agent": f"{USER_AGENT}/{VERSION}"}
...
ENDPOINT_MAP = {
    "pmi": {"ping": "/users/me", "cve": "cves"},
    "qsentry_feed": {"anon": "/files/anonymization", "mal_hosting": "/files/malicious_hosting"},
    "qsentry": {},
    "qwatch": {"ping": "/users/me", "exposures": "exposures"},
    "qauth": {},
}
...
def _search(**kwargs):
    ...
    req.set_proxy(proxies.get("http"), "http")
    req.set_proxy(proxies.get("https"), "https")
    ...
    while request_attempts < max_retries:
        try:
            ...
            raise Exception(f"API connection error {e!s}")
